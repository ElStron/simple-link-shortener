...
app.get('/:short_url', async (c) => {
  ...
  const link = await c.env.DB.prepare('SELECT url FROM links WHERE short_url = ?').bind(short_url).first()
  if (link) {
    await c.env.DB.prepare('UPDATE links SET clicks = clicks + 1 WHERE short_url = ?').bind(short_url).run()
